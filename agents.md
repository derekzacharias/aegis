...
- Auto-release behaviour is tenant-configurable from Settings › Security. The worker reads `OrganizationSettings.antivirusAutoReleaseStrategy` (fallback to `EVIDENCE_SCAN_AUTO_RELEASE_STRATEGY`) so analysts can opt into manual quarantine review, auto-return to pending, or auto-restore the prior evidence status.
...
- `/reports` endpoints cover queueing, listing, job detail, and PDF download. RBAC restricts mutations to analysts/admins while allowing read-only/auditor access to status and download routes. Queueing paths now confirm assessment ownership through Prisma before enqueuing work, and audit entries log which service user requested each render.
- `/assessments` lifecycle endpoints now operate entirely on Prisma: create seeds framework controls and progress counters, detail views include tasks and audit history, and mutations capture control status changes plus linked remediation tasks so agents can coordinate remediation directly from API responses.
...
- `/users/profile` surfaces the full profile CRUD path (read, update contact info, change password) with Prisma-backed audits. Agents that trigger notifications or impersonate reviewers should pull this payload immediately after login so timezone, job title, and phone data stay current in downstream messages; any automated profile mutation must include a descriptive `changes` blob to keep `UserProfileAudit` actionable.
- `/frameworks/:id/crosswalk` serves the control crosswalk engine. The API blends seeded mappings with cosine-similarity suggestions, supports manual overrides, enforces tag normalization, and stores evidence reuse hints so agents (or analysts) can reapply artifacts across frameworks without leaving the queue flow. Query filters now cover target framework scoping, search keywords, confidence thresholds, and status (`mapped` vs `suggested`) with paginated responses so automation clients can iterate large match sets deterministically.
- `/frameworks` exposes draft-aware endpoints (create, patch, control upsert, publish) so automation can bootstrap bespoke catalogues for each organisation. Prisma transactions roll back partial writes, publish enforces a non-empty control set, and metadata captures who promoted the framework so audit trails stay intact even for agent-driven updates.
- `/frameworks/:id/controls` now pulls from Prisma instead of static seeds, returning fully hydrated catalog entries (status rollups, linked assessments, evidence summaries, crosswalk mappings, and facet counts). Filters cover search, family, priority, control type, and the newly added assessment status dimension so agents can target gaps such as `UNASSESSED` or `UNSATISFIED` controls. The service automatically falls back to the in-memory seeds when the database is empty, which keeps developer sandboxes functional without Postgres.
  - Evidence objects include tags, upload timestamps, and framework coverage, allowing ingestion agents to triage stale or missing documentation programmatically. A forthcoming worker hook will listen for `framework.published` events and trigger crosswalk refresh jobs—see backlog entry below.
  - Control mappings expose confidence, origin (seed/algorithm/manual), tags, and evidence hints, so crosswalk or reuse agents can prioritise high-confidence relationships when suggesting remediation steps.
  - Facet payloads (family, priority, kind, status) make it cheap for automation to drive dashboard filters or schedule follow-up jobs without scanning the entire dataset.
  - See `docs/control-catalog.md` for end-to-end exploration and extension guidance that agents can reuse when seeding new frameworks or attaching synthetic data for tests.
...
- JWT-based auth replaces earlier API keys. Agents authenticate via `POST /api/auth/login`, store `accessToken` / `refreshToken` pairs, and call `/api/auth/refresh` ahead of expiry (observe `AUTH_ACCESS_TOKEN_TTL` / `AUTH_REFRESH_TOKEN_TTL`). Refresh failures should trigger full re-login and alerting so RBAC drift is caught quickly.
- Provision one service user per agent (e.g. `reports-agent@aegis.local`). The seed script now materialises an `ADMIN` bootstrap account that can create those identities; store generated passwords in your secret manager and update them with the `/api/users/password` endpoint when rotating credentials.
- Map agent behaviour to the narrowest viable role: READ_ONLY for telemetry collectors, AUDITOR for read-most / report-download bots, ANALYST for evidence mutation jobs, ADMIN only when orchestration has to manage roles or frameworks. This minimises blast radius if credentials leak.
- Profile context is hydrated post-login across API/web. Agents producing human-facing notifications should cache the profile response (name, job title, timezone, phone, role) to avoid redundant GET calls and ensure contact details stay consistent in outbound payloads.
